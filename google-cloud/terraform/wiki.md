...
| `service`    | Tên sản phẩm / dịch vụ (chữ thường, cho phép dấu gạch ngang)             | `safar` |
| `env`        | Môi trường (`dev` / `stg` / `prod` / `common`)                           | `prod` |


**Ví dụ:** `dx-safar-dev`, `dx-safar-prod`, `dx-safar-common`
...
| `service` | `safar`                                | Tên sản phẩm / dịch vụ                           |
...
gcloud projects list --filter="labels.service=safar"
```
...
| -------------------- | -------------------------- | ----------------------- |
| Cloud Run            | `{service}-{component}`    | `safar-app`              |
| Cloud SQL            | `{service}-{engine}-{env}` | `safar-mysql-prod`          |
| Cloud Build trigger  | `{service}-{branch}-build` | `safar-main-build`       |
| Artifact Registry    | `{service}-docker`         | `dx-safar-docker`           |
| Cloud Storage bucket | `{project-id}-{dùng cho}`  | `dx-safar-prod-assets` |
| VPC                  | `{project-id}-vpc`         | `dx-safar-prod-vpc`    |
...
| `service`    | プロダクト・サービス名（小文字英数字、ハイフン可）                | `safar` |
| `env`        | 環境識別子（`dev` / `stg` / `prod` / `common`） | `prod` |


**例:** `dx-safar-dev`、`dx-safar-prod`、`dx-safar-common`
...
| Cloud Run          | `{service}-{component}`    | `safar-app`              |
| Cloud SQL          | `{service}-{engine}-{env}` | `safar-mysql-prod`          |
| Cloud Build トリガー   | `{service}-{branch}-build` | `safar-main-build`       |
| Artifact Registry  | `{service}-docker`         | `dx-safar-docker`           |
| Cloud Storage バケット | `{project-id}-{用途}`        | `dx-safar-prod-assets` |
| VPC                | `{project-id}-vpc`         | `dx-safar-prod-vpc`    |
