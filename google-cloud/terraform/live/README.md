...
Single GCP project (`dx-safar`); environments separated by `env_suffix` in tfvars.
...
- Remote state bucket: `dx-safar-terraform-state` (in `live/*/terragrunt.hcl`).
- Keep `project_id` in `terraform.tfvars` aligned with `GCP_PROJECT_ID` in `load-env.sh` (`dx-safar`).
