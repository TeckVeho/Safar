...
output "mysql_root_password_secret_id" {
  description = "Secret Manager id for root@% password (admin CLI; not for SQL Studio)."
  value       = var.enable_cloud_sql ? google_secret_manager_secret.mysql_root_password[0].secret_id : null
}

output "mysql_studio_user_name" {
  description = "Built-in MySQL user for Cloud SQL Studio (host=%)."
  value       = var.enable_cloud_sql ? local.sql_studio_user_name : null
}

output "mysql_studio_password_secret_id" {
  description = "Secret Manager id for safar-studio@% password (SQL Studio login)."
  value       = var.enable_cloud_sql ? google_secret_manager_secret.mysql_studio_password[0].secret_id : null
}
