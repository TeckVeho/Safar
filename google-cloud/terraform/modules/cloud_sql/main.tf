...
# SQL Studio built-in auth requires a non-root user with host=% (not root@% or empty host).
resource "random_password" "sql_studio" {
  count = var.enable_cloud_sql ? 1 : 0
...
locals {
  ...
  mysql_root_password_secret_id  = substr("${local.name_prefix}-mysql-root-password-${var.env_suffix}", 0, 63)
  mysql_studio_password_secret_id = substr("${local.name_prefix}-mysql-studio-password-${var.env_suffix}", 0, 63)
  sql_studio_user_name           = "safar-studio"
}
...
resource "google_sql_user" "app" {
  ...
  # Empty host: Cloud Run unix socket only (not selectable in SQL Studio).
}

resource "google_sql_user" "studio" {
  count = var.enable_cloud_sql ? 1 : 0

  name           = local.sql_studio_user_name
  instance       = google_sql_database_instance.main[0].name
  host           = "%"
  password       = random_password.sql_studio[0].result
...
# root@% — admin CLI only; SQL Studio blocks root login.
resource "google_secret_manager_secret" "mysql_root_password" {
  count = var.enable_cloud_sql ? 1 : 0

  secret_id = local.mysql_root_password_secret_id
...
resource "google_secret_manager_secret_version" "mysql_root_password" {
  ...
  secret_data = random_password.sql_root[0].result
}

resource "google_secret_manager_secret" "mysql_studio_password" {
  count = var.enable_cloud_sql ? 1 : 0

  secret_id = local.mysql_studio_password_secret_id
...
resource "google_secret_manager_secret_version" "mysql_studio_password" {
  count = var.enable_cloud_sql ? 1 : 0

  secret      = google_secret_manager_secret.mysql_studio_password[0].id
  secret_data = random_password.sql_studio[0].result
}
