...
## Rename Terraform state bucket (e.g. `dx-kumu-common-terraform-state` → `dx-safar-terraform-state`)
...
```bash
gsutil mb -l asia-northeast1 -b on gs://dx-safar-terraform-state
gsutil -m cp -r gs://dx-kumu-common-terraform-state/** gs://dx-safar-terraform-state/
```
...
3. Apply again — Terraform creates/manages `dx-safar-terraform-state` only; the old bucket remains until you delete it manually after verifying remote stacks work.

4. Re-init env stacks if they still point at the old bucket (this repo uses `dx-safar-terraform-state` in `live/*/terragrunt.hcl`).
