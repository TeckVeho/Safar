variable "project_id" {
  type        = string
  description = "GCP project ID (e.g. dx-safar)."
}
...
variable "container_image" {
  type        = string
  description = "Full image URL (must already exist in Artifact Registry), e.g. asia-northeast1-docker.pkg.dev/PROJECT/REPO/safar-app:dev"
}
...
variable "enable_web" {
  type        = bool
  default     = true
  description = "Create the Next.js Cloud Run service (safar-app). Set false only to skip web in a disposable stack."
}
...
variable "project_iam_members" {
  ...
  default     = []
  description = <<-EOT
    Project-level IAM bindings. Scoped env custom roles (safarDevDeployer, etc.) auto-receive IAM Conditions
...
variable "sql_instance_name" {
  ...
  default     = ""
  description = <<-EOT
    Cloud SQL instance id (resource name). If empty, uses {project_id}-mysql-{env_suffix} (e.g. dx-safar-mysql-dev).
...
variable "sql_database_name" {
  ...
  default     = ""
  description = <<-EOT
    Logical database name inside the instance. If empty, uses {project_id}-mysql-{env_suffix} (e.g. dx-safar-mysql-dev).
...
variable "cloud_run_migrate_job_name" {
  type        = string
  default     = ""
  description = "Cloud Run Job name for `npx prisma migrate deploy`. If empty, uses dx-safar-migrate-{env_suffix}."
}
